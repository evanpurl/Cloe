        ...
        try:
            if self.values[0] == "Message Log":
                ...
                if ison[0] == 1:
                    ...
                    await interaction.response.edit_message(content=f"Message log **Disabled**", view=SelectView())
                else:
                    await dbset(interaction.guild.id, interaction.client.user.name, "ismsglogon", 1)
                    await interaction.response.edit_message(content=f"Message log **Enabled**", view=SelectView())
            if self.values[0] == "Report Transcript":
                ...
                if ison[0] == 1:
                    ...
                    await interaction.response.edit_message(content=f"Report Transcript **Disabled**", view=SelectView())
                else:
                    await dbset(interaction.guild.id, interaction.client.user.name, "isreporttranscripton", 1)
                    await interaction.response.edit_message(content=f"Report Transcript **Enabled**", view=SelectView())
