...
class patcmd(commands.Cog):
    ...
    async def pat(self, interaction: discord.Interaction, user: discord.User = None):
        try:
            if user is None:
                await interaction.response.send_message(content=f"""{self.bot.user.name} Pats your head <1102370970460770304>""", ephemeral=True)
            else:
                if user.id == self.bot.user.id:
                    await interaction.response.send_message(content=f"""I can't pet my own head, silly.""", ephemeral=True)
                else:
                    await interaction.response.send_message(content=f"""{self.bot.user.name} Pats {user.mention}'s head <1102370970460770304>""")
